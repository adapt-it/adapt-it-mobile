define(function (require) {
    ...
    return Backbone.View.extend({
        ...
        selectedAdaptation: function (event) {
            ...
            targetText = $(event.currentTarget.nextElementSibling).val();
            if (targetText.length > $(event.currentTarget.nextElementSibling).size) {
                $(event.currentTarget.nextElementSibling).size = targetText.length;
            }
        ...
        editAdaptation: function (event) {
            ...
            // resize the input field if needed
            targetText = $(event.currentTarget).val();
            $(event.currentTarget).size = (targetText.length > 20) ? targetText.length : $(event.currentTarget).size;
        },
        ...
        unselectedAdaptation: function (event) {
            ...
            if ($(event.currentTarget.previousElementSibling).html() !==
                    $(event.currentTarget).val().trim()) {
                // value has changed -- update
                // var pileid = $(event.currentTarget.parentElement).id;
                // var spid = pileid.substr(5, pileid.length - 5);
                // SourcePhrases.get(spid);
//                this.model.save();
            }
